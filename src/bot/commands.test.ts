...
import { handleIncoming, parseCommand, START_TEXT, stripMention, type Incoming, type Reply } from "./commands.js";
import { toIncoming } from "./telegram.js";
...
test("/palavra and a bare mention with no text explain how to use them", async () => {
  const s = setup();
  assert.match(textOf(await s.say("/palavra")), /Example: \/palavra/);
  assert.match(textOf(await s.say("@Palavra_paradevs_bot")), /Example: \/palavra/);
  assert.equal(s.llm.requests.length, 0);
});
...
test("a message with nothing to record gets a short answer", async () => {
  ...
  assert.match(textOf(await s.say("/palavra good morning")), /couldn't find a decision or a commitment/);
});
...
test("if the model cannot be reached the user gets an apology; other errors are not swallowed", async () => {
  ...
  assert.match(textOf(await s.say("/palavra x")), /couldn't process that message/);
...
test("/history needs a topic, says when nothing matches, is per group and escapes HTML", async () => {
  const s = setup();
  seedBudgetChain(s);
  assert.match(textOf(await s.say("/history")), /Example: \/history budget/);
  assert.equal(textOf(await s.say("/history <script>")), 'I found nothing about "&lt;script&gt;".');
